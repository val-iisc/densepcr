...
def batch_norm_template(input_data, is_training, name, moment_dims, bn_decay, scale_offset=True):

  with tf.variable_scope(name) as scope:
    shape = [input_data.get_shape()[-1]]
    pop_mean = tf.get_variable("moving_mean", shape, initializer = tf.constant_initializer(0.0), trainable=False)
    pop_var = tf.get_variable("moving_variance", shape, initializer = tf.constant_initializer(1.0), trainable=False)
    epsilon = 1e-3
#   decay = 0.999
    decay = bn_decay if bn_decay is not None else 0.9
    if scale_offset:
      scale = tf.get_variable("gamma", shape, initializer = tf.constant_initializer(1.0))
...
#     batch_mean, batch_var = tf.nn.moments(input_data, [0, 1, 2])
      batch_mean, batch_var = tf.nn.moments(input_data, moment_dims)
    ...
    output = tf.cond(is_training, lambda : bn_helper_train(), lambda : bn_helper_test())
    return output
